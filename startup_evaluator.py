...
Apply the provided evaluation framework rigorously: weights, thresholds, red flags, and evidence requirements.
If evidence is weak, score lower and clearly state what is missing.
Use concise, concrete rationales.
"""


def default_framework_config() -> dict[str, Any]:
    return {
        "pillar_weights": {
            "team": 0.25,
            "market_timing": 0.20,
            "product_value_prop": 0.15,
            "traction_pmf": 0.15,
            "business_model_unit_econ": 0.15,
            "deal_financing_risk": 0.10,
        },
        "score_scale": "1-5",
        "decision_thresholds": {
            "go_min_weighted": 3.5,
            "conditional_min_weighted": 3.0,
            "fatal_if_any_pillar_leq": 1,
            "minimum_pillar_scores": {
                "team": 3,
                "market_timing": 3,
                "product_value_prop": 3,
                "traction_pmf": 2,
                "business_model_unit_econ": 3,
                "deal_financing_risk": 3,
            },
        },
...
def load_input(path: str, max_website_chars: int) -> dict[str, Any]:
    ...
    framework.update(data["verification_context"].get("framework_config", {}))
    data["framework_config"] = framework
...
def build_user_prompt(data: dict[str, Any]) -> str:
    schema = {
        ...
        "pillar_scores_1_to_5": {
            "team": "int 1-5",
            "market_timing": "int 1-5",
            "product_value_prop": "int 1-5",
            "traction_pmf": "int 1-5",
            "business_model_unit_econ": "int 1-5",
            "deal_financing_risk": "int 1-5"
        },
        "readiness_focus": {
    ...
    prompt_payload = {
        ...
        "framework_config": data["framework_config"],
        "evaluation_priority": {
        ...
        "instructions": [
            ...
            "Use framework_config weights and thresholds to justify pillar and decision outputs.",
...
def apply_decision_gates(pillar_scores: dict[str, int], framework_config: dict[str, Any]) -> dict[str, Any]:
    thresholds = framework_config.get("decision_thresholds", {})
    weights = framework_config.get("pillar_weights", {})
    min_pillar = thresholds.get("minimum_pillar_scores", {})
    fatal_threshold = int(thresholds.get("fatal_if_any_pillar_leq", 1))
    go_min = float(thresholds.get("go_min_weighted", 3.5))
    conditional_min = float(thresholds.get("conditional_min_weighted", 3.0))

    weighted = 0.0
    for k, w in weights.items():
        weighted += float(w) * pillar_scores.get(k, 1)
    weighted = round(weighted, 2)

    fatal_flags = [k for k, v in pillar_scores.items() if v <= fatal_threshold]
    below_min = [k for k, minv in min_pillar.items() if pillar_scores.get(k, 1) < int(minv)]

    if fatal_flags:
        rec = "No-Go"
    elif weighted >= go_min and not below_min:
        rec = "Go"
    elif weighted >= conditional_min:
        rec = "Conditional Go"
    else:
        rec = "No-Go"

    conditions = [f"Improve pillar '{p}' to at least {min_pillar[p]}" for p in below_min][:3]
    return {
        "weighted_pillar_score_1_to_5": weighted,
        "recommendation": rec,
...
def normalize_result(result: dict[str, Any], framework_config: dict[str, Any]) -> dict[str, Any]:
    ...
    pillars_raw = result.get("pillar_scores_1_to_5", {})
    pillar_scores = {
        "team": clamp_1_5(pillars_raw.get("team", 1)),
        "market_timing": clamp_1_5(pillars_raw.get("market_timing", 1)),
        "product_value_prop": clamp_1_5(pillars_raw.get("product_value_prop", 1)),
        "traction_pmf": clamp_1_5(pillars_raw.get("traction_pmf", 1)),
        "business_model_unit_econ": clamp_1_5(pillars_raw.get("business_model_unit_econ", 1)),
        "deal_financing_risk": clamp_1_5(pillars_raw.get("deal_financing_risk", 1)),
    }
    decision_gate = apply_decision_gates(pillar_scores, framework_config)

    decision_raw = result.get("investment_decision", {})
    decision = {
        "recommendation": decision_gate["recommendation"],
        "weighted_pillar_score_1_to_5": decision_gate["weighted_pillar_score_1_to_5"],
        "fatal_flags": decision_gate["fatal_flags"],
        "conditions": decision_raw.get("conditions", decision_gate["conditions"])[:5],
    }
    ...
    return {
        "framework_config_used": framework_config,
        "scores": normalized_scores,
        "criteria_average_score": criteria_mean,
        "overall_score": weighted_overall,
        "pillar_scores_1_to_5": pillar_scores,
...
def write_markdown(path: str, final: dict[str, Any]) -> None:
    lines = [
        ...
        "",
        f"**Overall score (weighted 1-10):** {final['overall_score']}/10",
        ...
        f"**Decision:** {final['investment_decision']['recommendation']}",
        f"**Weighted pillar score (1-5):** {final['investment_decision']['weighted_pillar_score_1_to_5']}",
        "",
        "## Pillar scores (1-5)",
        "",
    ]
    for k, v in final["pillar_scores_1_to_5"].items():
        lines.append(f"- {k}: {v}/5")

    lines.extend(["", "## Decision conditions", ""])
    for c in final["investment_decision"].get("conditions", []):
        lines.append(f"- {c}")
    if final["investment_decision"].get("fatal_flags"):
        lines.append("- Fatal flags: " + ", ".join(final["investment_decision"]["fatal_flags"]))
...
def main() -> int:
    ...
    final = normalize_result(raw, framework_config=data["framework_config"])
    ...
    print(f"Decision: {final['investment_decision']['recommendation']}")
    return 0
